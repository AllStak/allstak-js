...
export interface AllStakVitePluginOptions
 ...
 * the runtime SDK.
 */
export function allstakVitePlugin(opts: AllStakVitePluginOptions = {} as AllStakVitePluginOptions): MinimalVitePlugin {
  ...
  return {
    ...
    async closeBundle() {
      ...
      try {
        ...
        // Don't fail the user's build over a sourcemap upload glitch.
        // The build artifact is still valid; the symbolicator will fall
        // back to filename-based lookup.
      }
    },
  };
}
...
export function _lastReport(plugin: MinimalVitePlugin): ProcessReport | null {
  // Plugins are returned by value, so this function is purely for ergonomics
  // in unit tests that want to assert what the plugin did. Real callers
  // should consume the report off `processBuildOutput` directly.
  void plugin;
  return null;
}
